...
# model parameters
max_len<-3 # look back 3 timesteps

# get vector of starting places, overlap by 3
...
hidden<- inputs %>% 
  layer_lstm(units=3,return_sequences=TRUE)%>%
  layer_lstm(units=3,activation="relu")

# output layer
outputs<- hidden %>% layer_dense(units=length(label))
...
# train
learningcurves <- model %>% fit(x = features_train,  y = labels_train, batch_size = 1, epochs = 100, verbose = 0)
...
evaluate(model,features_test,labels_test,verbose=0)
...
y<-t((t(fish[,2:4])*std[label])+mean[label])

# observed
res<-data.frame(year=fish$year,y)
colnames(res)<-c("year",paste(names(mean[label]),"obs",sep="_"))

# predicted
preds<-data.frame(year=fish$year[28:33],preds)
colnames(preds)<-c("year",paste(names(mean[label]),"pred",sep="_"))

# combine together
res<-left_join(res,preds,"year")

# long format
res<-res%>%pivot_longer(cols=2:7,names_to=c("area","season","abun","size","type"),
...
res%>%
  ...
  geom_line(size=1)+
  facet_grid(factor(size,levels=c("sml","med","lrg"))~.,scales="free_y")
```
