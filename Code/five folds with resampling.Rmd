...
# increase catch and compare predictions

```{r, eval = FALSE}

# adjust catch in large fish (age-5 and above) and rerun predictions for test data

# testing labels and features
test_labels <- select(testing,c(1,2,3)) 
test_data <- select(testing,c(4:ncol(testing)))

# increase catch for both regions age-5 and above by 5x
colnames(test_data)[c(124:128,133:137)]
colnames(feat_mns)[c(55:59,64:68)]
colnames(feat_stdevs)[c(55:59,64:68)]

# unscale
w<-t((t(test_data[,c(124:128,133:137)])*as.numeric(feat_stdevs[,c(55:59,64:68)]))+as.numeric(feat_mns[,c(55:59,64:68)]))
...
# model performance on test set
eval2 <- evaluate(model,test_data,test_labels,verbose=0)
...
resdat<-results
resdat$ID <- test_ind$ID
datdat2 <- left_join(alldata[,1:9],resdat,by="ID")

# carry over n abundance columns to observed abundance columns to resolve left_join NAs
datdat2$observed_small<-datdat$nsmall
datdat2$observed_medium<-datdat$nmedium
datdat2$observed_large<-datdat$nlarge

# compare predictions with catch and catch*5
bothresults<-cbind(datdat[6872:7764,13:15],datdat2[6872:7764,13:15])

plot(bothresults[,4]~bothresults[,1],xlim=c(0,10),ylim=c(0,10))
abline(a=0,b=1)

plot(bothresults[,5]~bothresults[,2],xlim=c(0,80),ylim=c(0,80))
abline(a=0,b=1)

plot(bothresults[,6]~bothresults[,3],xlim=c(0,25),ylim=c(0,25))
abline(a=0,b=1)
```