...
obs_pred_plot <- function(mod_summs_sf) {
  #Leave out the differences, plot
  mod_summs_sf %>% filter(type != "obs - pred") %>% 
  ggplot() +
...
strata_diffs_plot <- function(mod_summs_sf) {
...
# #Plotting each one on its own
# single_strata <- mod_diffs %>% 
#   split(.$STRATA) %>% 
#   map(function(x){
#     ggplot(x) + geom_sf(aes(fill = `prediction difference`), color = NA) + facet_wrap(~STRATA)  
#   })
# 
# 
# single_strata$`1260`  
