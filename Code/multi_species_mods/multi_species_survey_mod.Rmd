...
medium_q <- quantile(survey_data$nmedium, 0.95)
large_q <- quantile(survey_data$nlarge, 0.95)
...
upsample_recipe <- recipe_steps %>% 
  ...
  step_upsample(big_catch, over_ratio = 0.33, seed = 123, skip = FALSE)
...
survey_testing <- survey_testing %>% 
  ...
  select(ID, EST_YEAR, SEASON, SVVESSEL, tod, lunar_phase, 
         AREA, stratum, TOWDUR, BOTTEMP, anom, anom10, big_catch,
         nsmall, nmedium, nlarge) %>% 
  drop_na()
