...
### DROP Lagging Aggregate Survey Abundance
...
widedat <- left_join(moddf, trawltemp, by = "ID")
```

### DROP Annual/Monthly SST anomalies from previous year
...
## 1. Format Full Dataset

Format factor data, log-transform dependent vars.

```{r}
#Consolidate full dataset
survey_data <- widedat

# #Pull predictors we might want
# survey_data %>% 
#   select(nsmall, nmedium, nlarge,                                   #Dependent Vars
#          EST_YEAR, EST_MONTH, EST_TIME, SEASON, SVVESSEL,           #Temporal Information
#          DECDEG_BEGLAT, DECDEG_BEGLON,  AREA, stratum,              #Spatial Information
#          TOWDUR, BOTSPEED, BOTTEMP, SURFTEMP, AVGDEPTH,             #Tow information
#          anom, anom10,                                              #SST anomalies
#          AIRTEMP, BAROPRESS, WINDDIR, WINDDIR, WAVEHGT)             #Sea State
...
```{r}
model_recipe <-  recipe(nsmall + nmedium + nlarge ~                                      #Outcomes
                        #EST_YEAR + EST_MONTH + EST_TIME + 
                        SEASON + SVVESSEL +                                              #Temporal Information
                        AREA + stratum + DECDEG_BEGLAT + DECDEG_BEGLON +                 #Spatial Information
                        TOWDUR + BOTTEMP + SURFTEMP + AVGDEPTH +   #Tow information
                        anom + anom10 +                                                  #SST & SST 10 days prior
                        AIRTEMP + BAROPRESS + WINDDIR + WINDDIR + WAVEHGT,               #Sea State
...
### DROP SST at region level year i
...
### DROP Add trawltempprev

complete.cases on all data
...
### DROP begin and end year splits

Split alldata into testing and training data
