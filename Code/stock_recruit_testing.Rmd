...
### b. Manipulate un-standardized catch
...
#Make list containing test data for various scenarios
test_catch_scenarios <- map(catch_scenarios, function(x){
  
  #Adjust 2013 catches
  new_2013 <- test_data_df[year_labs == params$catch_adjust_year,] %>% 
    mutate(
      #Region 1
  ...
  same_1415 <- test_data_df[year_labs != "2013",] 
  
  #Bind them together
  catch_out <- bind_rows(list("2013" = new_2013, "2014-15" = same_1415))
...
names(test_catch_scenarios) <- str_c(catch_scenarios, " * Observed Catch")
```
...
test_catch_scenarios <- test_catch_scenarios %>% map(function(x){
...
### TESTING: Manipulate Standardized Catch Data

```{r, eval = FALSE}
# 
# # test_data   #Predictors
# # test_labels #Observed Responses
# 
# #Objective:
# #Adjust Catch of Adult Sized Fish 3+ in both regions for a specific year
# 
...
# 
# #Catch Scenarios
# catch_scenarios <- c(0.25, 0.5, 1 , 1.5, 2, 5, 10)
# 
# 
# test_catch_scenarios <- map(catch_scenarios, function(x){
#   
#   #Adjust 2013 catches
#   new_2013 <- test_data_df[year_labs == "2013",] %>% 
#     mutate(
#       #Region 1
#        c_r1_age_3     =  c_r1_age_3 * x,
#        c_r1_age_4     =  c_r1_age_4 * x,
...
test_catch_predictions <- test_catch_predictions %>% bind_rows(.id = "catch scenario") %>% 
  mutate(`catch scenario` = factor(`catch scenario`, levels = str_c(catch_scenarios, " * Observed Catch")),
         SEASON = factor(SEASON, levels = c("SPRING", "FALL")))

```

## Predicted Size Classes
...
small_plot <- ggplot(test_catch_predictions, 
  ...
  labs(x = NULL, y = "Predicted N-Small", 
       title = str_c("Adjusted", params$catch_adjust_year, "Commercial Landings of 3+ Age Fish"),
       subtitle = str_c("Impacts on predicted size classes \nRebalanced by sizeclass : ", params$rebalance)) +
  facet_wrap(~SEASON) + 
...
large_plot <- ggplot(test_catch_predictions, 
  ...
  facet_wrap(~SEASON) +
  guides(color = guide_legend(title = "2012 Catch Adjustment", title.hjust = 0.5, title.position = "top")) +
